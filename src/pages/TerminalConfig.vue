      ...
      <form @submit.prevent>
        ...
        <div class="row">
          <div class="col-md-4">
            <fg-input
              type="number"
              label="Isw Pending Notification Threshold"
              placeholder="Pending Notification Threshold"
              v-model="config.iswPendingNotify"
            >
            </fg-input>
          </div>
          <div class="col-md-4">
            ...
            <div class="p-4">
              <div class="row">
                <div class="form-group form-check">
                  <input
                    type="checkbox"
                    v-model="config.disableAll"
                    class="form-check-input"
                  />
                  <label class="form-check-label" for="exampleCheck1"
                    >Disable All Terminals</label
                  >
                </div>
              </div>
              ...
              <div class="row">
                <div class="form-group form-check">
                  <input
                    type="checkbox"
                    v-model="config.forceConfigAll"
                    class="form-check-input"
                  />
                  <label class="form-check-label" for="exampleCheck1"
                    >Force Configure All Terminals</label
                  >
                </div>
              </div>
              ...
              <div class="row">
                <div class="form-group form-check">
                  <input
                    type="checkbox"
                    v-model="config.upgradeAll"
                    class="form-check-input"
                  />
                  <label class="form-check-label" for="exampleCheck1"
                    >Upgrade All Terminals</label
                  >
                </div>
              </div>
              ...
              <div class="row">
                <div class="form-group form-check">
                  <input
                    type="checkbox"
                    v-model="config.allNibbsRoute"
                    class="form-check-input"
                  />
                  <label class="form-check-label" for="exampleCheck1"
                    >Route All to Nibbs</label
                  >
                </div>
              </div>
            </div>
        ...
        <div class="text-center">
          <p-button type="info" round @click.native.prevent="saveConfig2">
            <div
              class="spinner-grow"
              role="status"
              v-if="loading.show === true"
...
export default {
  ...
  methods: {
    ...
    submit() {
      this.$confirm("Save Configurations ?").then(() => {
        ...
        this.saveConfig({
          ...
          iswPendingNotify: this.config.iswPendingNotify,
        });
      });
    },
    saveConfig2() {
      this.$confirm("Save Config Panel! Are you Sure ?").then(() => {
        this.saveConfig({
  ...
  async mounted() {
    try {
      ...
      const res = await axios.get(
        `${process.env.VUE_APP_API_URL}/terminal/configuration/get`
      );
